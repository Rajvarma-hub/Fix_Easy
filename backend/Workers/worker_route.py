...
import calendar
from utilities.operations import cancel_job_by_worker, redis_client, otp_key
...
async def work_completed(inp: Workcompleted,db: Session = Depends(get_db),current_user = Depends(get_current_worker)):
    try:
        ...
        stored_otp = await redis_client.get(otp_key(inp.job_id))
        ...
        await redis_client.delete(otp_key(inp.job_id))
